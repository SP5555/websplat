...
fn cs_main(@builtin(local_invocation_id) thread_local_id : vec3<u32>,
    ...
    for (var i = 0u; i < idxCountInTile - 1u; i = i + 1u) {
        ...
        // some threads may do multiple comparisons to cover all pairs
        ...
        for (var j = 0u; j < compsPerThread; j = j + 1u) {
            ...
            let compIdx = compsPerThread * j + threadID;
            if (compIdx >= compsPerPass) {
                continue;
            }

            // leftIdx offsets by 0 or 1 depending on pass
            let leftIdx = compIdx * 2u + offset;
